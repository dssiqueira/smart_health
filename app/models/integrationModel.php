...
class IntegrationModel extends Model {
	...
	/*
	 * @method getIntegrationByUserIdAndAppId
	 * @author hguidi
	 * @param $uid, $appid, $deleted = null
	 * @return integration [id, app_id, user_id, token, deleted]
	 */
	...
	/*
	 * @method getAllIntegrationsByUserId
	 * @author hguidi
	 * @param $uid, $deleted = null
	 * @return integrations [id, app_id, user_id, token, deleted]
	 */
	public function getAllIntegrationsByUserId($uid, $deleted = null) {
		if (is_null ( $deleted )) {
			$sql = "SELECT id, app_id, user_id, token, deleted FROM integrations WHERE user_id = :user_id";
			$parameters = array (':user_id' => $uid);
		} else {
			$sql = "SELECT id, app_id, user_id, token FROM integrations WHERE user_id = :user_id AND deleted = :deleted";
			$parameters = array (
					':user_id' => $uid,
					':deleted' => $deleted 
			);
